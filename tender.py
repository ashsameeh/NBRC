...
data = pd.read_csv('mergedshorter.csv')

def identify_table_prompt(user_query):
    prompt = f"""
    You are a supply chain management assistant. You have access to a table called "merged_data" that contains comprehensive information from multiple sources about various tenders, their statuses, and related details. Use the following table schema to answer the user query.
    ...
    Please provide a detailed answer based on the available data.
    """
    return prompt

def make_output(user_query):
# query = "What is the acknowledgment status for tender with id NB32056724_21525?"
# query = "Did the FLEXPORT FREIGHT LLC accept tender NB32056724_21525?"
    detailed_prompt = identify_table_prompt(user_query)

# agent_executor = create_csv_agent(llm, 'masterdata.csv', verbose=True)
    agent_executor = create_csv_agent(llm, 'mergedshorter.csv', verbose=True)
